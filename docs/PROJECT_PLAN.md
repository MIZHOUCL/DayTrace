...
- 文件系统扫描：`roots` 下仓库外文件的当日改动（只记路径 / 时间 / 大小，见 ADR-019）
...
| 级别 | 内容 | 默认 |
|---|---|---|
| L0 | git 元数据（commit hash / message / author / 时间 / 分支）、`diff --stat` 行数；仓库外文件的路径 / mtime / 大小 | 开启 |
...
文件系统扫描的实测：3072 个文件 / 830 个目录约 0.4 秒（见 ADR-019）。初版列的「10 万文件元数据扫描不阻塞 UI」在 CLI 形态下不适用（没有 UI 可阻塞），改为「单次运行总耗时」这个用户唯一真正感知的指标。
...
| 变更 | 对应 ADR |
|---|---|
| 文件系统扫描：先取消、后按实测反馈重新纳入 | ADR-001 / ADR-019 |
