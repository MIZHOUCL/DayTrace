...
骨架已在仓库根目录（[`src/`](../src/)）跑通：零依赖 ESM JavaScript，43 个单元测试全绿，约 2030 行实现 + 576 行测试。21 条 P0 里 20 条已完成。

**2026-09-04 的一轮真实使用（Windows + JetBrains）暴露了四个 bug，已全部修掉**：① 会话被压成一条要点，几十条提问只出一行；② `git status --porcelain=v1 --no-color` 里 `--no-color` 是无效选项，而 `gitSafe` 把失败静默转成空串，导致**工作树证据自始至终恒为 0**；③ Codex 会话拿用户随口一句话当标题（出现「那几行呢？」这种条目）；④ 中文路径被 git 默认的 `core.quotePath` 转义成八进制。同时按 [ADR-019](./ADR.md) 把文件系统扫描重新纳入 —— 只看 git + AI 会话会漏掉没进 git 的目录里的全部工作。
