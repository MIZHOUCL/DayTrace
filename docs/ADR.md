...
状态：Accepted（范围部分已被 **ADR-019** 修订：文件系统扫描重新纳入）

决策：只读用户明确配置的位置 —— 配置的 `roots`（git 仓库 + 仓库外的文件改动）、`~/.claude/projects`、`~/.codex/sessions`。不支持全盘扫描。

原因：全盘扫描会造成隐私、性能与误报问题，也不符合「日志助手」而不是「监控软件」的定位。初版曾把「文件系统增量扫描」当作核心能力，本文档一度把它整体砍掉（理由是 git 已覆盖），实践证明那个判断过头了，见 ADR-019。
...
决策：所有时间戳按 UTC 存储；判定证据属于哪一天时，用可配置日界（默认本地时间 04:00）与半开区间 `[start, end)`。**时区默认跟随运行这台机器**，无需配置；需要固定时区时用 `--tz <IANA>` 或 `config.timezone`，实现方式是在任何日期计算之前设置 `process.env.TZ`，于是日界折算、归属日、会话时间戳解析全部自动一致，不需要把时区参数穿过每个函数。非法时区名立刻报错，绝不静默退回 UTC。生效时区会打印在 `daytrace where` 与日志页脚里，便于核对。`evidence` 表同时存 `occurred_at`（UTC）与 `local_date`（按日界折算），分组与搜索用后者。换时区后只重算 `local_date`，不改动原始时间戳。
...
## ADR-019：文件系统扫描重新纳入，与 git 分工

状态：Accepted（新增，修订 ADR-001 中「取消文件系统扫描」的部分）

决策：默认开启文件系统扫描，扫配置的 `roots` 下在当日窗口内 mtime 有变化的文件，**只记路径、时间、大小，绝不读正文**。与 git 的分工是硬规则：

- 落在任何已发现的 git 仓库内的文件**一律跳过**，交给 git 采集器（commit + 工作树状态）处理，避免同一份改动被记两遍；跳过的数量会在扫描摘要里报出来。
- 仓库外的文件才由文件扫描负责，按 `roots` 下的第一层目录归成「项目」。

边界与保护：`maxDepth` 默认 6、`maxFiles` 默认 5000（命中上限时显式报「已截断」，不静默丢弃）；跳过点文件与点目录；跳过体积黑洞（`node_modules`、`.venv`、`dist`、`target`、`Library`、`AppData`、`Program Files` 等）；跳过疑似敏感文件且连路径都不记录。敏感判定用精确规则（`^\.env(\..+)?$`、`\.(pem|key|p12|pfx|keystore)$`、`^id_(rsa|ed25519|…)`、`credentials?.(json|ya?ml)` 等）而不是 `secret*` / `token*` 前缀通配 —— 后者会误伤 `tokenizer.ts`、`secrets.example.json` 这类正常源码。可用 `--no-files` 或 `config.json` 的 `fileScan.enabled: false` 关闭。

原因：ADR-001 原先把这一层整体砍掉，理由是「git 已经覆盖文件变更」。真实使用推翻了这个判断 —— 作者在 Windows + JetBrains 环境下实测，当天大量工作发生在没有进 git 的目录里，只看 git + AI 会话时日志几乎是空的，用户直接的评价是「并没有什么价值」。git 只能覆盖「已经是仓库」的那部分，而「今天动过哪些文件」是日报的基础事实，不能依赖用户先把所有目录 git init。

代价与接受理由：mtime 精度、编辑器原子保存造成的假新增、非 git 目录的重命名检测这些问题依然存在，本版不解决 —— 因为只记「这个文件今天被改过」这一个事实，上述问题最多造成条目多一条少一条，不会产生错误的因果陈述。实测扫描 3072 个文件 / 830 个目录耗时约 0.4 秒，性能不是瓶颈。

## ADR-020：先按规则聚成「模块」，再交给用户勾选，最后才交给模型

状态：Accepted（新增，由真实使用反馈驱动）

决策：日志生成拆成三段流水线 —— **规则聚类 → 用户勾选 → 模型写作**。

1. **规则聚类**（`src/modules.js`）：按「项目 + 时间连续性」把当天证据聚成模块，同一项目内间隔 > 90 分钟即切块。每个模块带标题、类别（代码/文档/数据/杂项）、时间段、证据量、权重、以及默认是否选中。
2. **用户勾选**：模块是用户面对的最小选择单位。默认全选，用户剔除不想写进日记的（隐私、无关）。没有提问也没有提交、只有零星文件的项目自动并入「杂项」并默认排除。
3. **模型写作**：只把被选中模块的证据交给模型，写成日记，再跑既有的引用完整性校验。

配套的两条实现要求：
- 文件扫描改为**扩展名白名单**（`WORK_EXTENSIONS`）而不是继续扩充黑名单。
- 权重里文件数按 `3 * sqrt(files)` 计而不是线性。

原因：第一版把 175 条证据平铺输出，用户的评价是「可读性太差、无用信息太多、根本没有重点」，并明确指出想要的是「每天帮我写一篇日记，更详细、写的有根据」。纯规则写不出日记，但模型直接吃原始证据只会垃圾进垃圾出；而隐私筛选必须发生在发送之前，且只能由人来做。这三点合起来就决定了上面的流水线顺序。

黑名单换白名单的依据：实测那 175 条里绝大多数是 `.db-shm`、`NTUSER.DAT{...}.regtrans-ms`、`pg_internal.init`、`Local State`、`*.exe`、`*.lnk` 这类应用运行时状态，黑名单永远追不上。日志工具应当宁缺毋滥 —— 漏记一个冷门格式，比每天读 160 行垃圾好。放宽的出口是 `fileScan.extraExtensions` 与 `fileScan.mode: "all"`。

平方根计权的依据：QQ 的 62 个 `.db-shm` 若按线性计权会直接压过当天真正的周报工作。
